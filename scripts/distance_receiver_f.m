...
disp(t);
data_bit = message(t+38400:t+335999); %此处有6*4800 = 28800的冗余，因为之后需要准确判断前导码结束位置
figure(3);
plot(data_bit);
...
for i= half_window+1:1:n-half_window
    %进行峰值判断
    if impulse_fft_5k(i)>20 && impulse_fft_5k(i)==max(impulse_fft_5k(i-half_window:i+half_window))
...
% bias: 经验误差，需要实验测出
bias = -50000;
%##start各个值是开始录音的时间 t：真正信号从第t个采样点开始
%##开始收到信号的时间-sender发送信号的时间 = 单程传播需要的时间
%##bias是两个电脑本地时间的差异？
